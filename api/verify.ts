...
//   TAVILY_API_KEY
//
// NOTE: Use Node.js runtime so process.env works and TypeScript is happy.
...
Base your decision on the provided sources; do not fabricate links.`;
  ...
  if (
    !out ||
    !out.label ||
    !["GREEN", "YELLOW", "RED"].includes(out.label)
  ) {
    out = {
      label: "YELLOW",
      confidence: 0.5,
      summary:
        "Could not verify confidently. Review the sources manually.",
